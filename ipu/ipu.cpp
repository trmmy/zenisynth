...
{
    out("run");
    DialogBox(GetModuleHandle(NULL),MAKEINTRESOURCE(IDD_MAIN),m_hDummyWnd,(DLGPROC)mainWndProc);
}
...
void IPU::createWindow()
{
    ...
    m_hDummyWnd=CreateWindowEx(WS_EX_TOOLWINDOW,
                               strings::windowClass,
                               strings::windowTitle,
                               WS_OVERLAPPEDWINDOW,
                               CW_USEDEFAULT,0,
    ...
    if (!m_hDummyWnd)
    {
        MessageBox(NULL,_T("CreateWindowEx failed"),_T("CreateWindowEx failed"),MB_OK);
        ::exit(0);
    }
